...
### THREE MARKETS CANNOT BE CAPTURED FROM THIS MACHINE AT ALL

Las Vegas, San Antonio and Anaheim CITY COUNCIL answer only to a hosted runner.
`lasvegas.primegov.com` and `sanantonio.primegov.com` return HTTP 403 at 4,549
bytes to a Bangkok residential IP, and `local.anaheim.net` drops packets on both
ports with no RST. Measured from the GitHub runner (AS8075 Microsoft) on
2026-08-27: Las Vegas 200/187,325 bytes, `local.anaheim.net` 200/232,143 bytes.

ANAHEIM'S GRANICUS URL IS A REDIRECTOR, NOT A HOST, which is the part that reads
as reachable and is not. `AgendaViewer.php?view_id=2&clip_id=N` answers 302 with
...
```
npm run cadence:due       who is due today, plus the per-market capture verdict.
                          Writes cadence/due.json and cadence/verdict.json and
                          NOTHING to the database. cadence/ is gitignored.
npm run cadence:record    advances next_delivery from cadence/delivered.json.
                          Run AFTER the documents exist, never before: a date
                          advanced ahead of the generation is a week silently
                          skipped when the generation fails.
```

`.github/workflows/weekly.yml` is the whole cycle on `ubuntu-latest`, Monday
01:00 UTC. The document itself is generated by `dashboard/e2e/weekly.deliver.ts`,
Playwright project `deliver`, which SKIPS unless `CADENCE_RUN=1` so a local suite
run never mails anybody. Delivery is a workflow artefact plus a GitHub Issue; the
job fails on a red verdict so GitHub's own failure mail fires with nothing built
for it. Eight secrets, and the six required ones are checked BEFORE the capture.
...
14. A TRUNCATION TEST MAY NEVER COMPARE AGAINST A LIMIT THE CLIENT ASKED FOR.
    PostgREST answers with its own `db-max-rows`, 1000 on this project, whatever
    `.limit()` said. `report-build` asked for 2,000 events, was given 1,000, and
    tested `rows.length >= 2000` - so the flag that prints "What moved is not a
    complete list" could never be set, and a client document was short by 13
    events with nothing on the page to say so. Page the read with `.range()`
    until a short page and tie-break the order on `id`, because paging an ordered
    read whose sort key repeats serves one row twice and another never. Golden
    case `a-client-cap-the-server-overrode`.
