...
export async function projectFacetCounts(
  ...
  // THE FIFTH BYPASS IS GONE, and it was temporary by construction. While the
  // rename's tolerance window was open a module could be stored under either of
  // two names, and migration 017's function takes p_module as a single text
  // value compared with '=', so it would have counted the rows carrying the one
  // value it was handed while the list beside it showed both. Migrations 048 and
  // 049 have run, there is one value again, and the parameter carries it
  // correctly - so the RPC is back in use and the extra column read is not paid.
